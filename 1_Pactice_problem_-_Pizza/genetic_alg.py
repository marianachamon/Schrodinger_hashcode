...
#            gene[x] = randint(0, 7)
    ...
    if method == 'Reset':
        for x in range(random.randint(0,muatation_rate)):
            ...
            if mutated_individual[r] == 1:
                mutated_individual[r] = 0
            else:
                mutated_individual[r] = 1
...
def spawn_starting_population(amount):
    return [ [1 for i in range(0,len(slicesVec))]]+[spawn_individual() for x in range (1,amount)]

def spawn_individual():    
        return [random.randint(0,1) for x in range (0,len(slicesVec))]
...
#ITEMS = [Item(slicesVec[x], slicesVec[x]) for x in range(0,n) ]

popsize = 500;

pop = spawn_starting_population(popsize)
...
#        gen[-1]['Fitness'])/len(gen[-1]['Fitness']))
    fitness_max = np.append(fitness_max, maxGen )
#    res = open(Result_file, 'a')
#    res.write('\n'+str(gen[-1])+'\n')
#    res.close()
...
with open(path + "out.in","w") as f:
    f.write(str(len(bestInd))+"\n" + str(' '.join(map(str,bestInd))));  

endTime = datetime.now()

with open(path + endTime.strftime(" - %d-%m-%Y %H_%M - ") + "statics.in","w") as f:
