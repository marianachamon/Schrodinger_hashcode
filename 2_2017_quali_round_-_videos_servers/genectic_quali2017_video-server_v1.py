...
def isOverflow(individual):
    for i in range(0,len(individual)): 
        if np.sum(individual[i]) > X:
            print("Overflow!")
...
def fitness_calculation(individual):
    fitness_value = 0    
    individualc = overflowFix(individual)
    for i in range(0,len(req)):  
        ...
        for ci,lc in cachesEndp:
            if  req[i,0] in individualc[ci]: # if the video from req i is in one or more of the caches
...
def roulette(cum_sum, chance):
    veriable = list(cum_sum.deepcopy())
...
#        generation['Normalized Fitness']).cumsum()
    if method == 'Roulette Wheel':
        ...
        # generation is sorted in ascending fitness order
        # selected is organized in descending fitness order
...
def pairing(elit, selected, method = 'Fittest'):
    elit1 = elit
    selected1 = selected
    individuals = [elit1['Individuals']]+selected1['Individuals']
    fitness = [elit1['Fitness']]+selected1['Fitness']
...
def mating(parents1, method='Single Point'): #All parents have to have the same size
    parents =  parents1 
...
#            
    if method == 'Reset':
        for x in range(random.randint(0,muatation_rate)):
            r1 = random.randint(0,len(mutated_individual)-1)
            r2 = random.randint(0, len(mutated_individual[r1])-1)
            possibleGenes = list( set(range(0,V)) - set(mutated_individual[r1].copy()) )
            r3 = random.randint(0,len(possibleGenes)-1)
            mutated_individual[r1][r2] =  possibleGenes[r3]
#    isOverflow(mutated_individual)
...
print('hi1')
popu = spawn_starting_population(popsize-1, sizeInd)
print('hi2')
#pop = population(20,8,1,0)
gen = []
gen = first_generation(popu)
...
#        break
    ...
    if aux > maxGen:        
        maxGen = aux;
        bestInd =  list(  gen['Individuals'][np.argmax(gen['Fitness'])])
        print('\n-----Best ind:', bestInd)
        print('-----best fit:', maxGen,'\n')
